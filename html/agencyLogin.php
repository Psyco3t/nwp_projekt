...
<main class="page login-page">
    <section class="clean-block clean-form dark" style="background: #5eb1bfca;">
        ...
        if(isset($_GET['error']) and $_GET['error']==1) {
            echo '<div class="alert alert-warning text-primary alert-dismissible" role="alert" style="background: var(--bs-alert-border-color);padding: 20px 48px 16px 16px;"><button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button><span><strong>Wrong name or password entered.</strong><br /></span></div>';

        }
        ?>
        <?php
        if(isset($_GET['error']) and $_GET['error']==2) {
            echo '<div class="alert alert-warning text-primary alert-dismissible" role="alert" style="background: var(--bs-alert-border-color);padding: 20px 48px 16px 16px;"><button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button><span><strong>Your agency is not activated please contact administration</strong><br /></span></div>';

        }
        ?>
        <?php
        if(isset($_GET['error']) and $_GET['error']==3) {
            echo '<div class="alert alert-warning text-primary alert-dismissible" role="alert" style="background: var(--bs-alert-border-color);padding: 20px 48px 16px 16px;"><button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button><span><strong>Your agency is status is disabled please contact administration</strong><br /></span></div>';
        ...
        <div class="container">
            ...
            <form style="background: rgba(255,255,255,0.76);" action="../php/UAC/AgencyLogin.php" method="post">
                ...
                <div style="margin-bottom: 11px;padding-top: 0px;margin-top: -1px;overflow: visible;margin-left: 0px;display: inline-block;"><!--<a href="password_reset.php" style="display: inline-block;">I forgot my password</a>--></div><br><button class="btn btn-primary btn-tourDive-loginbtn"  type="submit">Login</button>
            </form>
        </div>
    </section>
</main>
